import flet as ft
from app.theme import PRIMARY, SECONDARY, CARD_BG, TEXT_SECONDARY, ERROR, SUCCESS, WARNING, border_all
...
def dashboard_view(page, navigate):
    ...
    rows_data = [
        (tr(page, "sales_month"), f"{currency}{d['sales_month']:,.0f}", ft.Icons.TRENDING_UP, "#2563EB", "/payments"),
        (tr(page, "invoiced_month"), f"{currency}{d['invoiced_month']:,.0f}", ft.Icons.RECEIPT, "#7C3AED", "/invoices"),
        (tr(page, "paid_month"), f"{currency}{d['paid_month']:,.0f}", ft.Icons.CHECK_CIRCLE, "#059669", "/payments"),
        (tr(page, "outstanding"), f"{currency}{d['outstanding']:,.0f}", ft.Icons.HOURGLASS_EMPTY, "#D97706", "/invoices"),
        (tr(page, "overdue"), f"{d['overdue_count']} ({currency}{d['overdue_amount']:,.0f})", ft.Icons.WARNING_AMBER, "#DC2626", "/invoices"),
        (tr(page, "stock_value"), f"{currency}{d['stock_value']:,.0f}", ft.Icons.INVENTORY, "#0891B2", "/products"),
        (tr(page, "low_stock"), str(d['low_stock']), ft.Icons.ERROR_OUTLINE, "#F59E0B", "/products"),
        (tr(page, "new_customers"), str(d['new_customers']), ft.Icons.PERSON_ADD, "#0D9488", "/customers"),
    ]

    status_bars = ft.Column(spacing=2)
    inv_total = max(d['total_invoices'], 1)
    for label, val, clr in [
        (tr(page, "draft_invoices"), d['draft_inv'], "#94A3B8"),
        (tr(page, "sent_invoices"), d['sent_inv'], "#3B82F6"),
        (tr(page, "partial_invoices"), d['partial_inv'], WARNING),
        (tr(page, "paid_invoices"), d['paid_inv'], SUCCESS),
    ]:
        ...
        ft.Container(
            content=ft.Column([
                ft.Row([
                    ft.Column([
                        ...
                        ft.Row([
                            ...
                            ft.Text(company.name if company else tr(page, "my_company"), size=13, color="rgba(255,255,255,0.85)"),
                ...
                ft.Row([
                    ft.Container(
                        content=ft.Column([
                            ft.Text(tr(page, "revenue"), size=11, color="rgba(255,255,255,0.6)"),
                    ...
                    ft.Container(
                        content=ft.Column([
                            ft.Text(tr(page, "outstanding"), size=11, color="rgba(255,255,255,0.6)", text_align=ft.TextAlign.RIGHT),
        ...
        ft.Tabs(
            ...
            content=ft.Column([
                ft.TabBar(
                    ...
                    tabs=[
                        ft.Tab(label=tr(page, "tab_main")),
                        ft.Tab(label=tr(page, "tab_kpi")),
                        ft.Tab(label=tr(page, "tab_signals")),
                        ft.Tab(label=tr(page, "tab_products")),
                    ],
                ),
                ft.TabBarView(
                    expand=True,
                    controls=[
                        ft.Container(
                            content=ft.Column([
                                ...
                                ft.Row([
                                    nav_card(tr(page, "customers"), ft.Icons.PEOPLE, "#059669", "/customers"),
                                    nav_card(tr(page, "products"), ft.Icons.INVENTORY_2, "#0891B2", "/products"),
                                ], spacing=10),
                                ft.Container(height=12),
                                ft.Row([
                                    nav_card(tr(page, "new_quote"), ft.Icons.ADD, "#D97706", "/quotes"),
                                    nav_card(tr(page, "invoices"), ft.Icons.RECEIPT, "#7C3AED", "/invoices"),
                                ], spacing=10),
                                ft.Container(height=8),
                                ft.Row([
                                    nav_card("Bons Livr.", ft.Icons.LOCAL_SHIPPING, "#2563EB", "/delivery_notes"),
                                    nav_card(tr(page, "payments"), ft.Icons.PAYMENTS, "#059669", "/payments"),
                                ], spacing=10),
                        ...
                        ft.Container(
                            content=ft.Column([
                                ...
                                ft.Container(height=12),
                                section_card(tr(page, "invoices_status"), ft.Icons.RECEIPT, "#7C3AED",
                                    status_bars,
                                ),
                            ], spacing=0, scroll=ft.ScrollMode.AUTO),
                            padding=ft.Padding(left=16, right=16, top=0, bottom=16), expand=True,
                        ...
                        ft.Container(
                            content=ft.Column([
                                ft.Container(height=6),
                                ft.Row([
                                    section_card(tr(page, "stock_signals"), ft.Icons.INVENTORY_2, "#0891B2",
                                        ft.Row([
                                            signal_item(tr(page, "low_stock_items"), d['low_stock'], WARNING),
                                        ]),
                                        ft.Row([
                                            signal_item(tr(page, "out_of_stock"), d['out_of_stock'], ERROR),
                                        ]),
                                        ft.Container(height=6),
                                        ft.Row([signal_item(tr(page, "below_reorder"), d['below_reorder'], "#F59E0B")]),
                                        ft.Container(height=8),
                                        ft.Text(tr(page, "top_selling"), size=11, weight=ft.FontWeight.BOLD, color="#94A3B8", height=14),
                                        ft.Container(height=4),
                                        top_selling_col,
                                    ),
                                    section_card(tr(page, "customer_signals"), ft.Icons.PEOPLE, "#059669",
                                        ft.Row([
                                            signal_item(tr(page, "active_customers"), d['active_customers'], SUCCESS),
                                            signal_item(tr(page, "overdue_customers"), d['overdue_customers'], ERROR),
                                        ], spacing=8),
                                        ft.Container(height=6),
                                        ft.Row([signal_item(tr(page, "total_customers"), d['total_customers'], "#3B82F6")]),
                                        ft.Container(height=8),
                                        ft.Text(tr(page, "top_customers"), size=11, weight=ft.FontWeight.BOLD, color="#94A3B8", height=14),
                                        ft.Container(height=4),
                                        top_cust_col,
                                    ),
                                ], spacing=10),
                                ft.Container(height=8),
                                ft.Row([
                                    section_card(tr(page, "quote_signals"), ft.Icons.DESCRIPTION, "#D97706",
                                        ft.Row([
                                            signal_item(tr(page, "draft_quotes"), d['draft_quotes'], "#94A3B8"),
                                            signal_item(tr(page, "validated_quotes"), d['validated_quotes'], SUCCESS),
                                        ], spacing=8),
                                        ft.Container(height=4),
                                        ft.Row([
                                            signal_item(tr(page, "sent_quotes"), d['sent_quotes'], "#3B82F6"),
                                            signal_item(tr(page, "confirmed_quotes"), d['confirmed_quotes'], "#7C3AED"),
                                        ], spacing=8),
                                        ft.Container(height=6),
                                        ft.Row([
                                            ft.Column([
                                                ft.Text(tr(page, "quotes_month"), size=10, color="#94A3B8"),
                                                ft.Text(str(d['quotes_month']), size=14, weight=ft.FontWeight.BOLD, color="#1E293B"),
                                            ], spacing=1, expand=True),
                                            ft.Column([
                                                ft.Text(tr(page, "amount_month"), size=10, color="#94A3B8", text_align=ft.TextAlign.RIGHT),
                                    ...
                                    section_card(tr(page, "invoice_signals"), ft.Icons.RECEIPT, "#7C3AED",
                                        ft.Row([
                                            signal_item(tr(page, "draft_invoices"), d['draft_inv'], "#94A3B8"),
                                            signal_item(tr(page, "sent_invoices"), d['sent_inv'], "#3B82F6"),
                                        ], spacing=8),
                                        ft.Container(height=4),
                                        ft.Row([
                                            signal_item(tr(page, "partial_invoices"), d['partial_inv'], WARNING),
                                            signal_item(tr(page, "paid_invoices"), d['paid_inv'], SUCCESS),
                                        ], spacing=8),
                                        ft.Container(height=4),
                                        ft.Row([signal_item(tr(page, "overdue_invoices"), d['overdue_count'], ERROR)]),
                                    ),
                                ], spacing=10),
                            ], spacing=0, scroll=ft.ScrollMode.AUTO),
                            padding=ft.Padding(left=16, right=16, top=0, bottom=16), expand=True,
                        ...
                        ft.Container(
                            content=ft.Column([
                                ft.Container(height=6),
                                ft.Row([
                                    section_card(tr(page, "top_products"), ft.Icons.INVENTORY, PRIMARY,
                                        top_prod_bars,
                                    ),
                                    section_card(tr(page, "sales_trend"), ft.Icons.SHOW_CHART, SECONDARY,
                                        sales_bars,
                                    ),
                                ], spacing=10),
                            ], spacing=0, scroll=ft.ScrollMode.AUTO),
