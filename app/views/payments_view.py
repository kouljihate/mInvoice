...
def payments_view(page, navigate):
    def load():
        ...
        rows.controls.clear()
        if not payments:
            rows.controls.append(ft.Text(tr(page, "no_payments"), color="#6B7280"))
        else:
            for p in payments:
    ...
    def edit_pay(pid, old_status):
        ...
        status_dd = ft.Dropdown(
            label=tr(page, "status"), value=p.status,
        ...
        dlg = ft.AlertDialog(
            ...
            content=ft.Column([
                card(ft.Column([
                    ft.Row([ft.Text(tr(page, "invoice"), weight=ft.FontWeight.BOLD, size=14, color="#6B7280"),
                            ft.Text(inv_label, size=14, color="#1A1A2E")], spacing=8),
                    ft.Row([ft.Text(tr(page, "amount"), weight=ft.FontWeight.BOLD, size=14, color="#6B7280"),
                            ft.Text(f"{p.amount:,.2f} MAD", size=14, weight=ft.FontWeight.BOLD, color=PRIMARY)], spacing=8),
                    ft.Row([ft.Text(tr(page, "date"), weight=ft.FontWeight.BOLD, size=14, color="#6B7280"),
                            ft.Text(p.date[:10], size=14, color="#1A1A2E")], spacing=8),
                    ft.Row([ft.Text(tr(page, "method"), weight=ft.FontWeight.BOLD, size=14, color="#6B7280"),
                            ft.Text(p.method, size=14, color="#1A1A2E")], spacing=8),
                    ft.Row([ft.Text(tr(page, "reference"), weight=ft.FontWeight.BOLD, size=14, color="#6B7280"),
                            ft.Text(p.reference or "-", size=14, color="#1A1A2E")], spacing=8),
                    ft.Row([ft.Text(tr(page, "notes"), weight=ft.FontWeight.BOLD, size=14, color="#6B7280"),
                            ft.Text(p.notes or "-", size=14, color="#1A1A2E")], spacing=8),
                    ft.Row([ft.Text(tr(page, "status"), weight=ft.FontWeight.BOLD, size=14, color="#6B7280"),
    ...
    page.add(page_layout(page, navigate, tr(page, "payments"), back_route="/dashboard",
