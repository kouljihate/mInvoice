...
def login_view(page, navigate):
    username = ft.TextField(label=tr_static(page.session.store.get("lang") or "en", "username"), prefix_icon=ft.Icons.PERSON, width=300,
                            border_radius=8, focused_border_color=PRIMARY)
    password = ft.TextField(label=tr_static(page.session.store.get("lang") or "en", "password"), prefix_icon=ft.Icons.LOCK, password=True,
        ...
        ft.Container(
            content=ft.Column([
                ft.Column([
                    ...
                    ft.Text(tr_static(page.session.store.get("lang") or "en", "mobile_invoicing"), size=28, weight=ft.FontWeight.BOLD, color=PRIMARY),
                    ft.Text(tr_static(page.session.store.get("lang") or "en", "manage_business"), size=8, color=TEXT_SECONDARY),
                    ...
                    ft.Button(
                        tr_static(page.session.store.get("lang") or "en", "login"), width=300, height=48, on_click=do_login,
